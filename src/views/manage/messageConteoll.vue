  ...
  <div class="main">
    ...
    <div class="input">
      <el-card shadow="always">
        ...
        <el-button type="primary" size="medium" @click="loadMessage">查询</el-button>
      ...
      <el-card shadow="always">
        <el-scrollbar height="100%">
            ...
            >
                ...
                <el-table-column property="username" label="评论人"  />
                <el-table-column property="parentId" label="父级ID"  />
...
export default {
  ...
  data() {
    return {
      ...
      rules: { // 与<el-form:rules="rules">form表单的父元素绑定
                content: [  //与<el-form-item prop="content">用户名的表单子元素项进行绑定
                    {required: true, message: '请输入内容', trigger: 'blur'}, // 给打开的弹窗写校验规则
                ],
                username: [  //与<el-form-item prop="username">用户名的表单子元素项进行绑定
                    {required: true, message: '请输入评论人', trigger: 'blur'}, // 给打开的弹窗写校验规则
                ],
                time: [  //与<el-form-item prop="time">用户名的表单子元素项进行绑定
  ...
  methods: {
    ...
    save(){
      ...
      if (valid) {
        API.request({
          url: this.form.id ? '/api/message' : '/api/message',
