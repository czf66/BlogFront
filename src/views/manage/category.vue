...
export default{
    ...
    data(){
        return{
            ...
            rules: { // 与<el-form:rules="rules">form表单的父元素绑定
                name: [  //与<el-form-item prop="name">用户名的表单子元素项进行绑定
                    {required: true, message: '请输入名称', trigger: 'blur'}, // 给打开的弹窗写校验规则
                ]
            },
            ids:[]
        }
    ...
    methods: {
        ...
        save(){
            ...
            if (valid) {
                API.request({
                    url: this.form.id ? '/api/category/update' : '/api/category/save',
